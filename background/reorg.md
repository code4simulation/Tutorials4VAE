...
*   **$p(z)$ (사전 확률, Prior)**:
    데이터 $x$를 관찰하기 전, 잠재 변수 $z$가 기본적으로 따를 것이라고 가정하는 확률 분포이다. VAE에서는 일반적으로 잠재 공간의 각 차원이 독립적인 표준 정규 분포를 따른다고 가정하며, 이는 잠재 공간을 규칙적이고 연속적으로 유지하려는 구조적 제약(Regularization) 역할을 수행한다.
